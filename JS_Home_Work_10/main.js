...
form1.onsubmit = function (e) {
    ...
    let obj_bloc = document.createElement('div');

    let ul_obj = document.createElement('ul');
    ...
    ul_obj.appendChild(name);
    ul_obj.appendChild(surname);
    ul_obj.appendChild(age);
    obj_bloc.appendChild(ul_obj);
    document.body.appendChild(obj_bloc);

    //=============Перевірка віку===============
    let userAge = parseInt(this.age.value);
    if (userAge < 18) {
...
let number_bloc = document.createElement('div');
number_bloc.classList.add('number_bloc');
number_bloc.style.width = '300px';
number_bloc.style.height = '300px';
number_bloc.style.background = 'yellow';
document.body.appendChild(number_bloc)

let h1_number = document.createElement('h1');
h1_number.innerText = ''

number_bloc.appendChild(h1_number)

let relogNumber = localStorage.getItem('number');
if (relogNumber) {
    let currentNumber = parseInt(relogNumber);
    let newNumber = currentNumber + 1;
    h1_number.innerText = newNumber.toString();
    localStorage.setItem('number', newNumber.toString());
} else {
    h1_number.innerText = '1';
...
// Отримати збережений масив сесій з локального сховища
...
sessions.forEach(session => {
    ...
    sessionItem.innerText = session.date;
    number_bloc.appendChild(sessionItem);
});
// =================================================================================================================
...
function renderItems() {
    ...
    next.disabled = endIndex >= locations.length;
}

function nextPage() {
    if (currentPage < Math.ceil(locations.length / itemsPerPage) - 1) {
        currentPage++;
        renderItems();
    }
}
...
if (!lastUpdateTime) {
    ...
    if (currentTime - lastUpdateTime >= 10000) {
        let currentPrice = parseInt(priceBlock.innerHTML);
        let updatedPrice = currentPrice + 10;
        priceBlock.innerHTML = `${updatedPrice} грн`;
